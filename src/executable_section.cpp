...
ExecutableSection::ExecutableSection(std::span<uint8_t const> data)
  ...
  if (::getenv("UREGEX_JITDUMP") != nullptr) {
    static JITDumper dumper{};
    dumper.record_load(data, m_executable);
  }
}

ExecutableSection::ExecutableSection(ExecutableSection &&other)
